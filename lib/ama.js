...
AMA.prototype.login = function(callback) {
	...
	req.pipe(concat(function(body) {
		...
		// Markers
		if ( self.markers.hash && self.markers.hash == data.markersHash) {
			self.markers.changed = false;
		} else {
			self.markers.hash = data.markersHash;
			self.markers.changed = true;
		}

		// Geofences
		if ( self.geofences.hash && self.geofences.hash == data.geofencesHash) {
			self.geofences.changed = false;
		} else {
			self.geofences.hash = data.geofencesHash;
			self.geofences.changed = true;
		}
